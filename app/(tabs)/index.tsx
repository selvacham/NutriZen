...
import { Plus, MessageCircle, LogOut } from 'lucide-react-native';
...
export default function DashboardScreen() {
    ...
    const handleAddWater = async () => {
        if (user?.id) {
            try {
                await addWaterLog(250, user.id); // 250ml glass
            } catch (error) {
                console.error('Error adding water:', error);
                Alert.alert('Error', 'Failed to log water. Please check your database setup.');
            }
        }
    };
            ...
            <SafeAreaView className="flex-1" edges={['top']}>
                ...
                >
                    ...
                    <View className="py-6 flex-row justify-between items-center">
                        ...
                        >
                            <LogOut size={20} className="text-rose-500 dark:text-rose-400" strokeWidth={2.5} />
                        </TouchableOpacity>
                    </View>
                    ...
                    <Animated.View entering={FadeInDown.delay(300).duration(500)} className="mb-8">
                        ...
                        <View className="bg-white dark:bg-slate-900 rounded-[32px] p-6 shadow-sm border border-slate-100 dark:border-slate-800">
                            ...
                            <View className="mb-6">
                                <View className="flex-row justify-between items-center mb-2.5">
                                    <Text className="text-base font-bold text-slate-800 dark:text-slate-200">Water</Text>
                                    <View className="flex-row items-center gap-3">
                                        ...
                                        <TouchableOpacity
                                            ...
                                            className="bg-blue-500 w-7 h-7 rounded-full items-center justify-center active:scale-90"
                                            activeOpacity={0.8}
                                        >
                                ...
                                <View className="h-2.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                                    <View
                                        ...
                                        style={{ width: `${Math.min(100, (waterConsumed / waterGoal) * 100)}%` }}
                                    />
                                </View>
                            </View>
